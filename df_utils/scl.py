...
class SingleCenterLoss(nn.Module):
    ...
    def forward(self, x, labels):
        ...
        batch_size = x.size(0)
        eud_mat = torch.sqrt(
            self.l2loss(x, self.C.expand(batch_size, self.C.size(0))).sum(
                dim=1, keepdim=True
            )
        )
        ...
        dist_real = (eud_mat * (1 - labels.float())).clamp(min=1e-12, max=1e12).sum()
        dist_fake = (eud_mat * labels.float()).clamp(min=1e-12, max=1e12).sum()
