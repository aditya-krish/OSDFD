...
class MixStyle(nn.Module):
    ...
    def __init__(self, p1=None, p2=0.5, alpha=0.1, eps=1e-6, mix="crossdomain"):
        """
        Args:
    ...
    def __repr__(self):
        return (
            f"MixStyle(p={self.p1}, alpha={self.alpha}, eps={self.eps}, mix={self.mix})"
        )
    ...
    def forward(self, x, index, train_mode):
        if not train_mode:
            return x
        ...
        x_sort = x[np.argsort(index)]
        x_pristine = x_sort[: B // 4]
        x_fake = x_sort[B // 4 :]

        mu = x_fake.mean(dim=1, keepdim=True)
        var = x_fake.var(dim=1, keepdim=True)
        sig = (var + self.eps).sqrt()
        mu, sig = mu.detach(), sig.detach()
        x_normed = (x_fake - mu) / sig

        lmda = self.beta.sample((x_fake.size(0), 1))
        ...
        perm = torch.arange(3 * B // 4)  # inverse index
        perm_a, perm_b, perm_c = perm.chunk(3)
        perm_a = perm_a[torch.randperm(B // 4)]
        perm_b = perm_b[torch.randperm(B // 4)]
        perm_c = perm_c[torch.randperm(B // 4)]
        if random.random() > self.p2:
            perm = torch.cat([perm_b, perm_c, perm_a], 0)
        else:
            perm = torch.cat([perm_c, perm_a, perm_b], 0)
        # else:
        #     raise NotImplementedError

        mu2, sig2 = mu[perm], sig[perm]
        if mu.size(0) != mu2.size(0):
            print(mu.size(), mu2.size(), lmda.size())
        mu_mix = mu * lmda + mu2 * (1 - lmda)
        sig_mix = sig * lmda + sig2 * (1 - lmda)

        mixed_x = torch.cat((x_pristine, x_normed * sig_mix + mu_mix))
        mixed_x_shuffle = mixed_x[index]
        return mixed_x_shuffle
...
class AdapterSuper(nn.Module):
    def __init__(self, embed_dims, reduction_dims, drop_rate_adapter=0):
        ...
        self.adapter_conv = Conv2d_cd(
            reduction_dims, reduction_dims, 3, 1, 1, theta=0.7
        )
    ...
    def forward(self, x, identity=None):
        ...
        # import pdb;pdb.set_trace()
        # out = self.linear(x, self.sampled_weight_0, self.sampled_bias_0)
        B, N, C = x.shape

        x_down = self.fc1(x)
        x_down = self.activate(x_down)
        x_patch = (
            x_down[:, 1:]
            .reshape(B, 14, 14, self.super_reductuion_dim)
            .permute(0, 3, 1, 2)
        )

        x_patch = self.adapter_conv(x_patch)

        x_patch = x_patch.permute(0, 2, 3, 1).reshape(
            B, 14 * 14, self.super_reductuion_dim
        )
        x_cls = (
            x_down[:, :1]
            .reshape(B, 1, 1, self.super_reductuion_dim)
            .permute(0, 3, 1, 2)
        )
        # x_cls = self.adapter_conv(x_cls)
        x_cls = x_cls.permute(0, 2, 3, 1).reshape(B, 1, self.super_reductuion_dim)
        x_down = torch.cat([x_cls, x_patch], dim=1)

        x_down = self.activate(x_down)
        x_down = self.dropout(x_down)
        x_up = self.fc2(x_down)  # equivalent to 1 * 1 Conv
...
class Attention(nn.Module):
    ...
    def set_sample_config(self, sample_LoRA_dim, sample_prefix_dim):
        ...
        if self.sample_LoRA_dim == 0:
            ...
            self.LoRA_a_weight = self.LoRA_a
            self.LoRA_b_weight = self.LoRA_b
    ...
    def forward(self, x):
        ...
        if self.LoRA_identity == False:
            ...
            # qkv_delta = F.linear(qkv_delta, self.LoRA_b_weight).reshape(B, N, 3, self.num_heads,
            #                                                             C // self.num_heads).permute(2, 0, 3, 1, 4)
            qkv_delta = (
                self.LoRA_b(qkv_delta)
                .reshape(B, N, 3, self.num_heads, C // self.num_heads)
                .permute(2, 0, 3, 1, 4)
            )
            q_delta, k_delta, v_delta = qkv_delta.unbind(
                0
...
class VisionTransformer(nn.Module):
    ...
    #             act_layer=None, weight_init='', super_prompt_tuning_dim=1024, super_LoRA_dim=1024,
    ...
    def __init__(
        ...
        self.mixstyle = MixStyle(p1=0.5, p2=0.5, alpha=0.1)
    ...
    def set_sample_config(self, config: dict):
        ...
        for i, blocks in enumerate(self.blocks):
            ...
            blocks.set_sample_config(
                ...
                sample_last_prompt_tuning_dim=self.sample_prompt_tuning_dim[i - 1]
                if i > 0
    ...
    def forward_features(self, x):
        ...
        if 0:  # self.sample_prompt_tuning_dim[0] != 0:
            visual_prompt_tokens = self.visual_prompt_token[
                :, : self.sample_prompt_tuning_dim[0], :
            ].expand(B, -1, -1)
            if not self.IS_not_position_VPT:
    ...
    def forward(self, x, index, mode):
        feat = self.forward_features(x)
        feat = self.mixstyle(feat, index, mode)
        if self.head_dist is not None:
            x, x_dist = self.head(feat[0]), self.head_dist(feat[1])  # x must be a tuple
